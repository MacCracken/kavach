...
- [ ] **aarch64: two raw x86 values left in `src/util.cyr`.** `file_write_secure_modal`'s `syscall(91, …)` runs `capset` on aarch64, so the requested mode is never applied. The `131072` open flag, in that function and in `file_write_secure_r`, is `O_LARGEFILE` there, so `O_NOFOLLOW` is lost. cyrius now provides `sys_fchmod` and a per-arch `O_NOFOLLOW`, and 6.6.6 warns at the fchmod site. The issue's third defect, `kv_sleep_ms`'s raw nanosleep, is fixed by the 6.6.6 pin (translated since 6.6.5). Measured under `qemu-aarch64 -strace` at v3.12.6; see `docs/development/issues/2026-09-12-raw-x86-syscall-numbers-on-aarch64.md`.
