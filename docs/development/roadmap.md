...
- [ ] **agnos: credential file injection opens with the Linux `sys_open` shape.** On agnos `sys_open` is `(name, namelen, AO_flags)`, so `file_write_secure_modal` (`src/util.cyr`) passes its flags as the name length there. `file_write_secure_r` is fine because it goes through the stdlib's `file_open` bridge; route this call the same way. (The OCI scratch-file opens have the same shape, but they sit behind `_oci_run`'s Linux-only guard.)
