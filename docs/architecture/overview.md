...
- Crypto via [sigil](https://github.com/MacCracken/sigil) 3.12.9 (pinned). Constant-time compare is now the stdlib `ct` module (`ct_eq_bytes_lens`) — sigil retired its own `ct_eq` in the 3.x line.
...
| Dep | Version | Purpose |
|-----|---------|---------|
| Cyrius toolchain | 6.5.27 (pinned in `cyrius.cyml`) | First-party tree is on the cc 6.5 line; the old 5.10.x sigil-NI asm-offset bisect is retired. Pin matches the installed cycc. ⚠ Since 6.5.26 a declared `[deps].stdlib` module that the resolver first reaches *transitively* is copied into `lib/` with **no `include` prepended** — see the stdlib row. |
| Cyrius stdlib | resolved by `cyrius deps` into `lib/` (gitignored) | `alloc, args, assert, async, bayan, bench, chrono, ct, dynlib, fdlopen, fmt, fnptr, freelist, fs, hashmap, hashmap_fast, io, keccak, mmap, net, process, random, result, sandhi, slice, str, string, syscalls, tagged, thread, thread_local, tls, vec`. The 6.2 line folded `json`/`base64` into `bayan` and retired `bigint`. ⚠ **`chrono` is also hand-included** in `src/util.cyr`, `tests/kavach.fcyr` and `tests/samay_integration.tcyr`: under cc 6.5.26+ it arrives transitively via `async` → `async_macos`, which suppresses its own top-level `include` and leaves every `clock_*` / `sleep_ms` symbol undefined. Declaring it is not sufficient — those includes are load-bearing (v3.11.14). |
| [sigil](https://github.com/MacCracken/sigil) | 3.12.9 (pinned tag) | SHA-256, HMAC-SHA256. Constant-time compare moved to the stdlib `ct` module (`ct_eq_bytes_lens`) — sigil retired `ct_eq` in 3.x. The 5.10.x SIGILL bisect that capped sigil at 2.9.0 no longer applies under cc 6.5.27. sigil 3.12.8 namespaced its error constructors `err_*` → `sigil_err_*`; kavach's own moved to `kavach_err_*` at v3.11.13 so the two no longer collide under last-def-wins. |
| agnosys | dropped at v3.5.0 | **No longer a dependency.** kavach internalized the Linux security backends it used (Landlock/seccomp = `security.cyr`, SELinux/AppArmor = `mac.cyr`, Linux audit = `kernel_audit.cyr`) as `src/` modules — the `agnosys → agnodrm` decomposition (agnosys narrowed to the device model; its security subsystems became agnodrm). See [`cyrius.cyml`](../../cyrius.cyml). |
