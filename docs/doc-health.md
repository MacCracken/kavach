...
|---|---|---|---|
| `README.md` | 2026-07-13 | ✅ Fresh | Status block gains a v3.7.1 entry (toolchain/dep refresh); older per-version blocks duplicate CHANGELOG. Build/deps sections refreshed to cc `6.4.62` / sigil `3.11.1` at v3.7.1 (agnosys already framed as dropped-at-v3.5.0); test/bench counts **422 / 22**. The deferred-feature forward-reference points to the roadmap. |
| `CHANGELOG.md` | 2026-05-10 | ✅ Fresh | v3.1.0 stanza records the modernization arc. v3.0.0 stanza unchanged. |
| `CLAUDE.md` | 2026-07-13 | ✅ Fresh | Version field bumped to v3.7.1; Language/pin notes updated to cc `6.4.62` / sigil `3.11.1` with agnosys noted as dropped at v3.5.0 (backends internalized). The 6.2 stdlib-consolidation hazard prose is kept as documented pin-move guidance. Cleanliness Check / Key Principles / DO-NOT list all carry the cyrius-shaped idioms. |
...
| `cyrius.cyml` | 2026-07-13 | ✅ Fresh | cc pin `6.4.62` (was 6.1.24 at v3.4.1; → 6.2.11 at v3.4.2; → 6.3.40 at v3.6.1; → 6.4.62 at v3.7.1), sigil pin `3.11.1` (was 3.7.14), **no agnosys dep** (dropped at v3.5.0 — backends internalized), stdlib list mirrors src/. `json`→`bayan` and `bigint`-dropped since the 6.2 consolidation; `ct`/`keccak`/`thread`/`thread_local`/`async` stay declared (opt-in modules — a missing one SIGILLs at runtime). Comment blocks reworded to `agnosys (agnodrm)` at identity sites at v3.7.1. |
...
| `getting-started.md` | 2026-07-13 | ✅ Fresh | § "Build + install" carries the `cyrius deps` step + `lib/`-gitignored model; toolchain line refreshed to cc `6.4.62` (pin matches installed cycc) at v3.7.1; sigil `3.11.1` with the constant-time-via-`ct` note and the agnosys dependency **corrected to dropped-at-v3.5.0** (backends internalized; was wrongly documented as a live 1.4.3 pin). Build → configure → execute walkthrough still tracks src/. |
