...
|---|---|---|
| ✅ **Fresh — touched in the v3.1.0 arc or the post-cut sweep** | 9 | `CHANGELOG.md`, `VERSION`, `cyrius.cyml` (new), `.github/workflows/{ci,release}.yml` (ci.yml gained the 4095-byte `[deps]`-window gate at v3.11.14), `docs/development/roadmap.md`, `docs/doc-health.md` (this file) — all from the modernization arc. Plus `README.md`, `CLAUDE.md`, `docs/guides/getting-started.md`, `docs/development/rust-old-removal.md` — refreshed in the post-cut sweep. As of v3.11.14 these carry the cyrius-`6.5.27` / sigil-`3.12.9` reality with agnosys dropped at v3.5.0 (was cyrius-`6.5.21` through v3.11.13; cyrius-`6.4.62` / sigil-`3.11.1` through v3.7.1; cyrius-`6.2.11` / sigil-`3.7.14` / agnosys-`1.4.3` through v3.4.2; cyrius-`6.0.43` / sigil-`3.5.9` through v3.4.0); all still lib-via-`cyrius deps`. |
...
|---|---|---|---|
| `README.md` | 2026-08-17 | ✅ Fresh | Status block rewritten for **v3.11.14**: cc `6.5.21 → 6.5.27`, deps re-verified as already latest, and the dropped-`chrono`-include story with its 6.5.26 bisect. §Build pin lines and the stdlib dep bullet now warn that declaring a module is not sufficient — the hand-includes are load-bearing. Test/bench counts hold at **684 / 25**. Earlier per-version status blocks keep their own (correct) historical pins. Build/deps sections refreshed to cc `6.5.21` / sigil `3.12.9` at v3.11.13; test/bench counts **684 / 25**. The §Build cleanliness block no longer tells the reader to run `cyrius audit` — that is the *toolchain's* self-host audit, not a project gate; it now names `fmt`/`lint`/`vet` individually. The "Known integration caveats" bullet was rewritten: the `err_*` overlap is resolved (both sides namespaced) and the surviving `syserr_*`/`agnosys_*`/`path_exists`/`attestation_result_new` overlaps are no longer described as "benign". Constructor-API note updated for `kavach_err_*`. Older per-version status blocks duplicate CHANGELOG and are left as history. The deferred-feature forward-reference points to the roadmap. |
| `CHANGELOG.md` | 2026-08-17 | ✅ Fresh | **v3.11.14** stanza records the pin move, the `chrono` resolver bug (mechanism + bisect + upstream filing), the nine-release-stale demo banner, and the benchmark set — including the CONTENDED first row that was discarded rather than published. v3.1.0 stanza records the modernization arc. v3.0.0 stanza unchanged. |
| `CLAUDE.md` | 2026-08-17 | ✅ Fresh | Version field → **v3.11.14**; pin → cc `6.5.27` everywhere (Language line, DO-NOT pin-bump rule, DO-NOT fmt rule). Pin-move hazards went from three to **four** — "a declared stdlib module reached transitively is never included" joins opt-in-stdlib, stdlib-consolidation and symbol-collision, with the standing instruction to read a new `undefined function` on a stdlib symbol as hazard (4) first. Version field bumped to v3.11.13; Language/pin notes updated to cc `6.5.21` / sigil `3.12.9` with agnosys noted as dropped at v3.5.0 (backends internalized). Pin-move hazards went from two to **three** — symbol collision with sigil joins opt-in-stdlib and stdlib-consolidation, since cyrius auto-prepends every `[deps.*]` module and resolves a shared name by source order with only a warning. The 6.2 stdlib-consolidation hazard prose is kept as documented pin-move guidance. Cleanliness Check / Key Principles / DO-NOT list all carry the cyrius-shaped idioms. |
...
| `VERSION` | 2026-08-17 | ✅ Fresh | ⛔ This row read `3.7.1` for four minors while the file said otherwise — corrected at v3.11.14.  Now **`3.11.14`**. `3.7.1` — single source of truth, read into `cyrius.cyml` via `${file:VERSION}`. |
| `LICENSE` | (initial commit) | 🔵 Evergreen | GPL-3.0-only. |
| `cyrius.cyml` | 2026-08-17 | ✅ Fresh | cc pin → `6.5.27` (was 6.5.21 at v3.11.13). Deps unmoved — sigil `3.12.9`, samay `1.0.1`, ai-hwaccel `2.3.16` all re-verified against upstream tags as already latest. The `[deps]` block gained a long ⛔ note: declaring a stdlib module does **not** guarantee its `include`, with the `cbt/deps.cyr:521`-vs-`:582` mechanism, the chrono instance, the "do not fix by reordering" reasoning, and the upstream filing path. ⛔ **The file is now STRUCTURED, not just commented**: that block lives BELOW the array because writing it above pushed `[deps]` from byte 3676 to 5288, past the 4095-byte prefix `_auto_deps` scans — which silently disabled dep prepending entirely. A 4-line pointer sits above the array; headroom is **299 bytes** and CI gates it. Do not move commentary back above `[deps]`. cc pin `6.5.21` (was 6.1.24 at v3.4.1; → 6.2.11 at v3.4.2; → 6.3.40 at v3.6.1; → 6.4.62 at v3.7.1; → 6.5.21 at v3.11.13), sigil pin `3.12.9` (was 3.11.1), ai-hwaccel `2.3.16` + samay `1.0.1` optional behind the default-on `scheduler` feature, and the `[deps.sigil]` comment block now records the `err_*` collision history so a future bump does not reintroduce a bare `err_*`, **no agnosys dep** (dropped at v3.5.0 — backends internalized), stdlib list mirrors src/. `json`→`bayan` and `bigint`-dropped since the 6.2 consolidation; `ct`/`keccak`/`thread`/`thread_local`/`async` stay declared (opt-in modules — a missing one SIGILLs at runtime). Comment blocks reworded to `agnosys (agnodrm)` at identity sites at v3.7.1. |
