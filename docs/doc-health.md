...
|---|---|---|---|
| `README.md` | 2026-09-24 | ✅ Fresh | Status block for **v3.12.6** on top (incl. the aarch64 credential-write fixes). §Build → cc `6.6.6`, 730 assertions, `fmt --check` (plain `cyrius fmt` rewrites in place; the old "writes to stdout" note was wrong), `cyrius distlib --all`. Deps list → sigil `3.12.18` (from the toolchain snapshot) plus samay / ai-hwaccel; stdlib list completed (`atomic` / `math` / `sakshi`). §Consume: the documented consumer stdlib set lacked `sakshi` and no longer built — corrected and re-verified end to end on cc 6.6.6; sigil-from-snapshot noted. Older per-version status blocks are history. |
| `CHANGELOG.md` | 2026-09-24 | ✅ Fresh | **v3.12.6** stanza: pin move with the audit-chain torn-write A/B; the audit chain's all-or-nothing append (`_audit_append`, from the torn-tail task) with its before/after measurements and the cases it does not cover; the aarch64 raw-syscall fixes with before/after strace; agnos `sys_open` → `file_open`; dep table with the sigil-snapshot explanation; the stale confine-bundle fix; the manifest trim; the retired 4095 gate; verification; benchmark deltas with toolchain attribution, plus the four rows shown to move with code placement. |
...
| `005-v2-hardening-pass.md` | 📦 Frozen — accepted, amended | P(-1) hardening pass — 9 CWE-class fixes. The mitigation set is implemented and tested. C4's audit-log clause carries a superseded-in-part note (v3.12.6): the log is created 0600 by the open and `fchmod`ed on the fd, not appended with `file_append_locked` and `chmod`ed by path. |
...
| File | Last touched | Status | Notes |
|---|---|---|---|
| `roadmap.md` | 2026-09-24 | ✅ Fresh | Header → **v3.12.6** / cc `6.6.6` / sigil `3.12.18` / samay `1.1.5` / ai-hwaccel `2.4.0`. The completed "Moving the cyrius pin to 6.6.6" section was removed (its record is CHANGELOG 3.12.6). Tech debt carries the torn records the audit rollback cannot reach and the undiagnosed aarch64 fork/exec failures under qemu-user; the aarch64 raw-syscall and agnos `sys_open` items were fixed in 3.12.6. The stale `cyrius fmt` clean item was dropped: it shipped in 3.11.15 (drift drained, CI Format check made blocking) and its pinned-`6.0.43` caveat was out of date; `fmt --check` re-verified 0 drift across CI's 51 files on cc `6.6.6`. Future-facing only since v3.4.0. |
