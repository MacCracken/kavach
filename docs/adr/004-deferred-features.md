...
| 3 | Seccomp / Landlock / cgroups | **Shipped:** cgroups v2 (3.2.0); seccomp in the exec child (3.9.0; architecture-checked since 3.12.8); Landlock's full filesystem right set (3.11.1), `IOCTL_DEV` (ABI v5) and the abstract-unix and signal scopes (ABI v6) since 3.13.2. **Not enforced:** Landlock's network rules (ABI v4): the TCP port fields are counts with no port list, and since 3.13.2 they are not scored. The jailer's `setresuid` is 3.15.x. |
