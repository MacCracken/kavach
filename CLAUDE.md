...
- **Language**: Cyrius (pinned at `6.4.62` in `cyrius.cyml`). The 5.10.x sigil-NI asm-offset bisect that capped the tree at `5.10.44` no longer applies under cc 6.x — the first-party tree is on the cc 6.4 line (sigil 3.11.1; agnosys was dropped at v3.5.0, its security backends internalized as `src/` modules — the `agnosys → agnodrm` decomposition). Two live pin-move hazards: (1) **opt-in stdlib modules** — since sigil 3.6, `ct`/`keccak`/`thread`/`thread_local` are not in the cyrius auto-prepend union and must be in `[deps]` + reachable, or a crypto path SIGILLs (exit 132) on a `ud2` trap at runtime (see CHANGELOG 3.4.1/3.4.2); (2) **the 6.2 stdlib consolidation** — the standalone `json`/`base64` modules were folded into `bayan` and `bigint` was retired (sigil bundles its own `u256`/`u384`), so a 6.1→6.2 move must re-point `[deps]` (`json` → `bayan`, drop `bigint`) or `cyrius deps` fails to resolve (see CHANGELOG 3.4.2). Validate a pin move with a clean `deps → build → lint → vet → test → bench` run rather than the old asm-offset bisect.
- **Version**: SemVer, v3.7.1 (Cyrius port; Rust v1.x/v2.x archived in git history pre-3.1.1)
...
- **Do not bump the Cyrius pin** without a clean `deps → build → lint → vet → test → bench` run on the new toolchain across the first-party tree. The pin is `6.4.62`; the old 5.10.x sigil-NI asm-offset bisect is retired (the tree is on cc 6.4 now). A pin move can require migrating renamed/retired stdlib + sigil symbols (e.g. sigil's `ct_eq` → stdlib `ct_eq_bytes_lens` in the 3.x line — CHANGELOG 3.3.0), **declaring newly opt-in stdlib modules** in `[deps]` (e.g. `thread_local`, `async` — CHANGELOG 3.4.1), and **re-pointing consolidated stdlib modules** (`json`/`base64` → `bayan`, drop retired `bigint` — CHANGELOG 3.4.2); a missing opt-in module builds clean but SIGILLs at runtime, and a retired module name makes `cyrius deps` fail outright.
- **Do not run `cyrius fmt` against a non-pinned local toolchain** — fmt output is minor-version-sensitive; running a non-`6.4.62` fmt and committing creates new drift against CI's `6.4.62`. (The pin currently matches the installed `cycc`; if `cycc` later drifts ahead of the pin, skip fmt writes until the pin catches up.)
